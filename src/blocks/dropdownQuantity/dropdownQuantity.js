...
export class DropdownQuantity extends Block {
	constructor(options = {}) {
		const { attr, title, placeholder, elements, onValuesChange } = options;
		const template = require('./dropdownQuantity.pug');
		require('./dropdownQuantity.sass');
		...
		this.controls = new Container({
			attr: { class: 'dropdownQuantity_controls' },
		});
